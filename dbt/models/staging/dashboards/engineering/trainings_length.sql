...
WITH d_dataset AS (
    ...
    INNER JOIN d_data dd ON dd.dataset_id = COALESCE(pmc.new_dataset_id, lpmc.aiblock_id) AND dd.created_at < ts.start_time
    GROUP BY
        ts.new_id
...
SELECT
    ...
    COALESCE(data_count, 0) + COALESCE(legacy_data_count, 0) AS total_data_count,
    data_count,
    legacy_data_count,
